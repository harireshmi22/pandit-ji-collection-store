...
export async function POST(req) {
  ...
  try {
    ...
    const productIds = [...new Set(orderItems.map((item) => item.product?.toString?.()).filter(Boolean))];

    const dbProducts = await Product.find({ _id: { $in: productIds } }).lean();

    const productMap = new Map(dbProducts.map((p) => [p._id ? String(p._id) : 'unknown', p]));
    ...
    for (const item of orderItems) {
      ...
      if (!item.product) {
        return NextResponse.json({ message: 'Product ID is required for all order items' }, { status: 400 });
      }
